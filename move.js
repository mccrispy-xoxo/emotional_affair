...
scale.onmousedown = function (e) {
  ...
  // 鼠标滚轮滑动图片旋转
  fa.onwheel = function (e) {
    e.preventDefault();
    var delta = Math.max(-1, Math.min(1, e.wheelDelta || -e.detail)); // 获取滚轮的滚动方向
    var rotation =
      parseInt(
        box.style.transform.replace("rotate(", "").replace("deg)", "")
      ) || 0; // 获取当前旋转角度

    // 根据滚轮滚动方向进行旋转
    rotation += delta * 10; // 调整旋转速度
